<template>
    <nav ref="navRoot" class=" navbar navbar-expand-lg classic transparent navbar-light">
        <div class="container flex-lg-row flex-nowrap align-items-center">
          <div class="navbar-brand w-100">
            <a href="./">
              <img src="../assets/logo-dark.png" srcset="../assets/img/logo-dark@2x.png 2x" alt="">
            </a>
          </div>
...
import { initDropdownHover, initTooltips, initPopovers, initOffcanvas } from '@/utils/ui'
...
onMounted(() => {
  const root = navRoot.value || document
  cleanups.push(initDropdownHover(root))
  cleanups.push(initTooltips(root))
  cleanups.push(initPopovers(root))
  cleanups.push(initOffcanvas(root))

  // Lắng nghe cuộn để dính + đổi style
  window.addEventListener('scroll', onScroll, { passive: true })
  onScroll() // set trạng thái đúng ngay khi vào trang
})
...
<style scoped>
/* Nếu muốn dính: dùng sticky, không cần fixed */
.navbar { transition: background-color .2s, box-shadow .2s, padding .2s; }
.navbar.is-stuck {
  position: sticky; top: 0; z-index: 1030;     /* dính trên cùng */
  background-color: rgba(255,255,255,0.95);    /* đổi nền khi cuộn */
  box-shadow: 0 4px 12px rgba(0,0,0,.06);
}

/* (Tùy chọn) hiệu ứng logo/height nhỏ lại khi cuộn */
.navbar.is-stuck .navbar-brand img { transform: scale(.92); transition: transform .2s; }
